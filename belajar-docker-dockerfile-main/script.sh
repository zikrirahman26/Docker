...
# FROM Instruction
docker build -t khannedy/from from #build docker image dari folder from

docker image ls

# RUN Instruction
docker build -t khannedy/run run #build docker image dari folder run

docker build -t khannedy/run run --progress=plain --no-cache #display output nya

docket image ls |grep khannedy #list docker image yang berjalan

# CMD Instruction
docker build -t khannedy/command command

docker image inspect khannedy/command #detail dari image
...
culr localhost:8080
#perintah permintaan HTTP menggunakan utilitas curl untuk menampilkan responsnya
...
# Docker Push Digital Ocean Cloud Provider
